...
public class DrivetrainSubsystem extends SubsystemBase {
  ...
  public Command setVoltagesCommand(DoubleSupplier left, DoubleSupplier right) {
    return Commands.run(() -> this.drivetrain.setVoltages(left.getAsDouble(), right.getAsDouble()));
  }
  ...
  public Command setVoltagesArcadeCommand(DoubleSupplier drive, DoubleSupplier steer) {
    return this.run(() -> {
      ...
      drivetrain.setVoltages(speeds.left, speeds.right);
    });
  }

  /** Creates a new DrivetrainSubsystem. */
  public DrivetrainSubsystem() {
    drivetrain = new DrivetrainIOTalonSRX();
  }
